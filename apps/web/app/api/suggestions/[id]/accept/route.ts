...
export async function POST(
  ...
  try {
    ...
    const rateLimitResult =
      await suggestionAcceptRateLimit.limit(organizationId);
    ...
    try {
      if (suggestion[0].type === "whitelist") {
        ...
        if (!customerId) {
          ...
          console.warn(
            "[suggestion_accept] Cannot blacklist without customerId",
            {
      ...
      if (detectionContext) {
        await trackFeedback(organizationId, validatedSuggestionId, "accepted", {
          detectionId: suggestion[0].detectionId,
