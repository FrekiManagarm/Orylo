...
2. **AC2**: AI identifies common fraud patterns: unusual amounts (>2x or <0.1x average), geo mismatches (IP≠card country), velocity spikes (>10 tx/hour), card testing patterns
...
- [x] **Task 1**: Create rule recommendation engine (AC1, AC2, AC3)
  ...
  - [x] Identify fraud patterns:
    ...
    - [x] Geo mismatches: IP country ≠ card country (extracted from detector results)
...
- [x] **Task 5**: UI component for recommendations (AC4, AC5)
  - [x] Create `components/ai-rule-recommendation-card.tsx` component
    ...
    - [x] Confidence badge: Shadcn Badge, variant based on confidence (high ≥0.8: 'default', medium 0.5-0.8: 'secondary', low <0.5: 'outline')
...
├── lib/
│   └── ai/
│       ├── rule-recommendation-engine.ts    # Recommendation generation
│       ├── rule-impact-simulator.ts          # Impact preview
│       └── merchant-stats-analyzer.ts        # Transaction pattern analysis
├── app/
│   ├── api/
│   │   ├── organizations/
│   │   │   └── [id]/
...
│   │       └── [id]/
│   │           ├── apply/
│   │           │   └── route.ts              # POST apply rule
│   │           └── preview/
│   │               └── route.ts              # POST preview impact
│   └── (authenticated)/
│       └── settings/
│           └── custom-rules/
│               └── page.tsx                  # Settings page
└── components/
    ├── ai-rule-recommendation-card.tsx       # Recommendation UI
    └── rule-impact-preview.tsx               # Impact preview dialog
```
...
- **Tests**:
  ...
  - Apply rule → rule created and tracked

### Testing Standards
- Framework: Vitest (unit), Playwright (E2E)
- Coverage target: ≥80% for recommendation logic
...
const mockFraudDetections = [
  ...
  // - Geo mismatches (IP ≠ card country)
...
**Security**: ✅ Zod validation, rate limiting, Better Auth session, RLS checks (ADR-010 compliant)  
**Task Sequence**: ✅ Logical order with clear dependencies  
**Anti-Hallucination**: ✅ All technical claims verified against codebase, data extraction clarified  
**Implementation Readiness**: ✅ Self-contained context, clear instructions, ADR references added

### Issues Resolved

**Should-Fix Issues (All Corrected)**:
1. ✅ Settings route clarified (new page `/settings/custom-rules`, separate from `/settings/stripe`)
2. ✅ Data extraction from detectorResults clarified (IP country, card country, velocity from detector metadata)
3. ✅ Velocity field source clarified (from `detectorResults` metadata `txCount` or calculated)
4. ✅ Zod validation schemas added for all API inputs (ADR-010)
5. ✅ Rate limiting specified for all endpoints (ADR-010)
6. ✅ Better Auth session verification and RLS checks added
7. ✅ UI design specifications added (colors, responsive, accessibility)
8. ✅ Detailed mock data structure provided for tests (fraud_detections, ai_rule_recommendations, custom_rules)
9. ✅ ADR references added (ADR-003, ADR-006, ADR-010)
...
- ✅ Created database schema `ai_rule_recommendations` in `packages/database/src/schema/ai-rule-recommendations.ts`
- ✅ Generated migration file: `packages/database/drizzle/0003_chief_whiplash.sql`
- ✅ Created rule recommendation engine with pattern analysis (AC1, AC2, AC3)
- ✅ Created rule impact simulator for preview (AC5)
- ✅ Created API endpoints with Zod validation and rate limiting (AC4, AC5)
- ✅ Created UI components: recommendation card and Settings page (AC4, AC5)
- ✅ Added navigation link in app-sidebar.tsx
- ✅ Implemented caching strategy (AC7)
- ⚠️ Note: Migration needs to be applied to database (run `drizzle-kit push` or apply migration)
- ⚠️ Note: Rule effectiveness tracking (Task 6) requires Trigger.dev scheduled job (deferred - can be implemented later)
