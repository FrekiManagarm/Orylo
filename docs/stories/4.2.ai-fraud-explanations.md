...
8. **AC8**: Cost control: Rate limiting (10 explanations/minute per organization via Redis), caching similar detections (same detector pattern → reuse explanation)
...
- [x] **Task 6**: Fallback template explanations (AC7)
  ...
  - [x] Format: "Cette transaction a été signalée par: [detector names]. Score de risque: [score]"

- [x] **Task 7**: Cost control & caching (AC8)
  - [x] Rate limiting: Max 10 explanations/minute per organization
  - [x] Cache similar detections (same detector pattern) → reuse explanation
  - [x] Monitor API costs (PostHog event: `ai_explanation_generated`, `tokens_used`)
  - [ ] Alert if monthly cost >€200 (TODO: Requires PostHog integration)
...
**Prompt Template**:
```
Tu es un expert en détection de fraude. Analyse cette détection et explique en français (2-3 phrases) pourquoi cette transaction a été signalée.
...
const detectorResultsFormatted = detectorResults
  .map(d => {
    const reason = d.reason || d.metadata?.reason || 'Détecteur déclenché';
    return `- ${d.detectorId}: Score ${d.score}/100, Décision: ${d.decision} (${reason})`;
  })
  .join('\n');

// Example output:
// - velocity: Score 85/100, Décision: BLOCK (Trop de transactions en peu de temps)
// - geolocation: Score 60/100, Décision: REVIEW (Pays inhabituel)
// - trust-score: Score 25/100, Décision: BLOCK (Score de confiance faible)
```
...
- Fallback to templates if budget exceeded (>€200/month)
...
- Verify explanation belongs to user's organization (via `detectionId` → `fraud_detections.organizationId`)
...
├── trigger/
│   └── jobs/
│       └── ai-explanation.job.ts      # Trigger.dev job
├── lib/
│   └── ai/
│       ├── explanation-prompt.ts      # Prompt building
│       └── explanation-templates.ts    # Fallback templates
├── app/
│   └── api/
│       └── detections/
│           └── [id]/
...
└── components/
    ├── ai-explanation.tsx             # Explanation display component
    └── detection-details-dialog.tsx   # Integrate explanation
```
...
- Coverage target: ≥80% for explanation logic
...
const mockAIExplanation = {
  ...
  detectionId: 'det_001',
  organizationId: 'org_001',
  explanation: 'Cette transaction a été signalée car plusieurs facteurs de risque ont été détectés. Le détecteur de vélocité a identifié un nombre anormalement élevé de transactions (10) dans un court laps de temps (1 heure), ce qui suggère un possible test de carte. De plus, le score de confiance du client est faible (25/100), indiquant un historique de transactions suspectes.',
...
const mockOpenAIResponse = {
  choices: [
    {
      message: {
        content: 'Cette transaction a été signalée car...',
      },
    },
  ],
  usage: {
...
**Template Compliance**: ✅ All required sections present, no placeholders  
**File Structure**: ✅ Clear file paths and source tree context  
**UI Completeness**: ✅ Design specifications added (colors, responsive, accessibility, SSE integration)  
**AC Coverage**: ✅ All 8 acceptance criteria covered by tasks  
**Security**: ✅ Zod validation, rate limiting, XSS prevention, Better Auth session (ADR-010 compliant)  
**Task Sequence**: ✅ Logical order with clear dependencies, integration point clarified  
**Anti-Hallucination**: ✅ All technical claims verified against codebase, Trigger.dev integration point corrected  
**Implementation Readiness**: ✅ Self-contained context, clear instructions, ADR references added

### Issues Resolved

**Should-Fix Issues (All Corrected)**:
1. ✅ Trigger.dev integration point clarified (from `saveFraudDetection()` in `detect-fraud.ts`, not webhook route)
2. ✅ AC6 aligned with ADR-008 (SSE with backend polling 5s, not frontend polling 1s)
3. ✅ Zod validation schemas added for API inputs (ADR-010)
4. ✅ XSS sanitization with DOMPurify added for explanation display (ADR-010)
5. ✅ UI design specifications added (colors, responsive, accessibility, SSE integration)
6. ✅ Detailed mock data structure provided for tests (fraud_detections, ai_explanations, LLM responses)
7. ✅ ADR references added (ADR-006, ADR-008, ADR-010)
8. ✅ DetectorResults format detailed with examples
...
- ✅ Created database schema `ai_explanations` in `packages/database/src/schema/ai-explanations.ts`
- ✅ Generated migration file: `packages/database/drizzle/0002_jittery_hulk.sql`
- ✅ Created Trigger.dev job for async AI explanation generation (AC1, AC2)
- ✅ Integrated job trigger in `saveFraudDetection()` (non-blocking, fire-and-forget)
- ✅ Created API endpoint with Zod validation and rate limiting (AC4)
- ✅ Created UI component with DOMPurify sanitization and SSE integration (AC4, AC5)
- ✅ Implemented fallback template explanations (AC7)
- ✅ Added cost control: rate limiting (10/min), caching similar patterns (AC8)
- ⚠️ Note: Migration needs to be applied to database (run `drizzle-kit push` or apply migration)
- ⚠️ Note: Trigger.dev environment variables required (TRIGGER_SECRET_KEY, TRIGGER_PROJECT_ID)
- ⚠️ Note: OpenAI API key required (OPENAI_API_KEY)
- ⚠️ Note: PostHog cost alert (€200/month) requires PostHog integration (deferred)
